...
function GitHubMultiple({ repos }) {
  return (
    <div className="relative inline-block text-left">
      <Menu>
        <>
          <Menu.Button className="text-nord-3 dark:text-nord-4 dark:hover:text-white hover:text-black p-2 hover:bg-nord-4  dark:hover:bg-nord-0 rounded focus:ring focus:ring-nord-8 focus:outline-none">
          ...
          >
            <Menu.Items className="absolute -right-2 w-56 mt-2 origin-top-right bg-white border border-gray-200 divide-y divide-gray-100 rounded-md shadow-lg outline-none z-20">
              <div className="py-1">
                {repos.map((item) => (
                  <Menu.Item key={item}>
                    {({ active }) => (
